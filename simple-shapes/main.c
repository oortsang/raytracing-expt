...
bitmap_t *sphere_render(Camera *cam, Sphere *sphere, int rpp) {
    ...
    DoubleColor *dpixels = calloc(bm->width*bm->height, sizeof(DoubleColor));
    ...
    /* Quantize at the end */
    bm->pixels = calloc(bm->width*bm->height, sizeof(Color));
    for (int pi = 0; pi < bm->height; pi++) {
        for (int pj = 0; pj < bm->width; pj++) {
            int idx = pi*bm->width+pj;
            /* Is it being copied properly? */
            bm->pixels[idx] = col_from_double(dpixels[idx]);
        }        
    }

    if (rays) free(rays);
    if (dpixels) free(dpixels);
    return bm;
}
...
int main(int argc, char **argv) {
    ...
    Camera cam = {.fr=(VecH){0,0,-1,1}, .n=(VecH){0,0,-1,0}, .v=(VecH){0,1,0,0}, .u=(VecH){1,0,0,0},
                  .d=1., .nc=0.5, .fc=1.5, .fov=90, .px=400, .py=300, .ar=4./3};
    printCamera(&cam);
    ...
    Sphere sphere = {.center={0,0,0,1}, .col=(DoubleColor){128,0,0}, .r=0.5};
    DoubleColor col;
    ...
    bitmap_t *bm = sphere_render(&cam, &sphere, 3);
    /* printf("bm->pixels=%p\n", bm->pixels); */
